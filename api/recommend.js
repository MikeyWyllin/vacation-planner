...
Return this JSON shape:
{
  ...
  "picks": [
    ...
    if (!aiRes.ok) {
      res.status(200).json({ mode: 'no_ai_picks', message: 'OpenAI request failed. No suggestions are shown until AI processes the trip.', detail: data.error?.message || data });
      return;
    }
    ...
    if (!parsed?.picks?.length) {
      res.status(200).json({ mode: 'no_ai_picks', message: 'AI did not return parseable picks. No suggestions are shown until AI processes the trip.', raw: text });
      return;
    }

    res.status(200).json({ mode: 'ai', ...parsed });
  } catch (err) {
    res.status(200).json({ mode: 'no_ai_picks', message: 'AI backend error. No suggestions are shown until AI processes the trip.', detail: err.message });
  }
}
