...
function buildQuestions() {
  return [
    {
      id: 'duration', type: 'single', text: 'How long do you want to be gone?',
    ...
    {
      id: 'distance', type: 'single', text: 'How far are you willing to go?',
      helper: 'These options are generated from your zip code, trip length, and travel method. This is where the app starts thinking geographically.',
    ...
    {
      id: 'sleepBudget', type: 'single', text: 'What is your sleep-only budget per night?',
    ...
    {
      id: 'tripPace', type: 'single', text: 'What pace do you want?',
    ...
    {
      id: 'crowds', type: 'single', text: 'How do you feel about crowds?',
    ...
    {
      id: 'fameLevel', type: 'single', text: 'Do you want famous or hidden?',
...
function render() {
  ...
  $('rangeStat').textContent = state.answers.distance?.label || (state.origin ? `~${estimateDriveLimit()} mi` : '—');
...
function renderQuestion(q) {
  ...
  $('questionType').textContent = q.type === 'multi' ? 'Multi select' : (q.type === 'text' ? 'Text answer' : 'Single choice');
...
function chooseAnswer(q, value) {
  if (q.type === 'single') {
    ...
    if (q.id === 'travelModes') delete state.answers.distance;
  }
  render();
}
...
function scoreDestination(dest) {
  ...
  const modes = answerArr('travelModes');
  const distanceAnswer = state.answers.distance;
...
function renderResults(results, mode = 'local') {
  ...
  const modeText = mode === 'ai' ? 'AI chose these from your answers, the 500-place database, and live web search.' : 'Fallback local ranking is showing because the AI backend did not return final picks.';
...
async function submitTrip() {
  ...
  $('submitBar').classList.add('hidden');
  $('resultsGrid').innerHTML = '';
  $('resultsSummary').textContent = `AI is reading your answers and checking the 500-place database. Database size: ${META.us} U.S. + ${META.world} worldwide.`;
  $('aiLoading').classList.remove('hidden');
  const local = localRank();
  ...
  try {
    ...
    if (data.mode === 'ai' && Array.isArray(data.picks) && data.picks.length) {
      const merged = data.picks.slice(0, 3).map((pick, idx) => mergeAiPick(pick, local[idx], scored));
      renderResults(merged, 'ai');
    } else {
      renderResults(local, 'local');
      $('resultsSummary').textContent = `AI did not return final picks, so this is fallback local ranking. ${data.message || ''} ${data.detail || ''}`.trim();
    }
  } catch (err) {
    console.info('AI backend unavailable. Showing local ranking.', err);
    renderResults(local, 'local');
    $('resultsSummary').textContent = `AI backend error, so this is fallback local ranking: ${err.message}`;
  } finally {
    $('aiLoading').classList.add('hidden');
  }
}

function mergeAiPick(pick, fallback, scored) {
  ...
  return {
    ...
    distance: pick.distance ?? found.distance ?? fallback?.distance,
