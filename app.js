...
function renderResults(results, mode = 'local') {
  ...
  const modeText = mode === 'ai' ? 'AI refined the picks with your answers and live-search style reasoning.' : 'Local 500-destination ranking is showing now. AI will refine it when the backend is connected.';
...
async function submitTrip() {
  ...
  try {
    const scored = DB.map(d => scoreDestination(d)).sort((a,b) => b.score - a.score).slice(0, 35);
    ...
    console.info('AI backend unavailable. Staying on local ranking.', err);
  } finally {
...
function mergeAiPick(pick, fallback, scored) {
  const found = scored.find(x => normalize(x.name) === normalize(pick.name)) || fallback || {};
  return {
    ...found,
    ...pick,
    ...
    distance: found.distance ?? pick.distance,
...
$('copyPromptBtn').addEventListener('click', async () => {
  ...
  try { await navigator.clipboard.writeText(text); $('copyPromptBtn').textContent = 'Copied'; }
  catch { $('copyPromptBtn').textContent = 'Prompt Ready'; }
  setTimeout(() => $('copyPromptBtn').textContent = 'Copy Prompt', 1600);
});
